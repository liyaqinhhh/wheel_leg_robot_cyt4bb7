...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     非阻塞串口发送数组（仅写 TX FIFO，不等待发送完成）
// 参数说明     uart_n          串口模块号
// 参数说明     *buff           要发送的数组地址
// 参数说明     len             发送长度
// 返回参数     uint32          已发送字节数（可能少于 len，取决于 FIFO 剩余空间）
// 使用示例     uart_write_buffer_fifo(UART_1, buf, 64);
// 备注信息     适用于中断上下文中快速发送，不阻塞等待 TX Complete
//-------------------------------------------------------------------------------------------------------------------
uint32 uart_write_buffer_fifo(uart_index_enum uart_n, const uint8 *buff, uint32 len)
{
    uint32 sent = 0;
    uint32 fifo_size = Cy_SCB_GetFifoSize(scb_module[uart_n]);
    while (sent < len)
    {
        if (Cy_SCB_GetNumInTxFifo(scb_module[uart_n]) >= (fifo_size - 1))
            break;
        Cy_SCB_WriteTxFifo(scb_module[uart_n], *buff);
        buff++;
        sent++;
    }
    return sent;
