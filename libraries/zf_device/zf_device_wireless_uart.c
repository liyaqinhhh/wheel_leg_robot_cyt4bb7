...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     无线转串口模块 非阻塞发送字符串（中断安全）
// 参数说明     *str            要发送的字符串地址
// 返回参数     uint32          剩余未发送字节数 0=全部发送成功
// 使用示例     wireless_uart_send_string_nonblock("hello");
// 备注信息     不等待 RTS 忙、不逐字节等 TX Complete，仅写 FIFO
//-------------------------------------------------------------------------------------------------------------------
uint32 wireless_uart_send_string_nonblock(const char *str)
{
    if (NULL == str)
        return 0;
    uint32 len = strlen(str);
    ...
    while (remaining > 0)
    {
        uint32 chunk = (remaining > 30) ? 30 : remaining;
        uint32 sent = uart_write_buffer_fifo(WIRELESS_UART_INDEX, (const uint8 *)str, chunk);
        str += sent;
        remaining -= sent;
        if (sent < chunk)
            break; /* FIFO 将满，剩余数据下次再发 */
        /* 发完一包后再次检查 RTS */
        if (gpio_get_level(WIRELESS_UART_RTS_PIN))
            break;
    }
