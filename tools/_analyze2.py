...
path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'docs', 'telemetry_turn_test_20260622_210221.csv')
...
# Auto-detect phases by target_yaw changes
phase_starts = [0]
prev_ty = ty[0]
for i in range(1, n):
...
for k in range(len(phase_starts)-1):
    ...
    if e - s < 5:
        continue
    label = 'TY=%.1f' % ty[s]
    print('\n%s [%d:%d] %dpts: pitch %.1f~%.1f roll %.1f~%.1f yaw %.1f~%.1f gzpk %.0f ot %d~%d ogp %d~%d mL %d~%d mR %d~%d' % (
        label, s, e, e-s, min(p[s:e]), max(p[s:e]), min(rl[s:e]), max(rl[s:e]),
        min(y[s:e]), max(y[s:e]), max(abs(v) for v in gz[s:e]),
        min(ot[s:e]), max(ot[s:e]), min(ogp[s:e]), max(ogp[s:e]),
        min(ml[s:e]), max(ml[s:e]), min(mr[s:e]), max(mr[s:e])))
