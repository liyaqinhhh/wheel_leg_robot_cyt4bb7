...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      注册自定义事件回调到事件表
//  @param      event_id    事件标识号
//  @param      cb          事件回调函数指针, NULL 则忽略
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
/*
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      按 event_id 删除事件表中所有匹配条目
//  @param      event_id    要删除的事件标识号
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      按 event_id 查找事件回调函数
//  @param      event_id    事件标识号
//  @return     InsSeg_EventCallback 回调函数指针, NULL 表示未找到
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      保存单个段数据到其对应的 Flash 页
//  @param      idx     段索引 (0~INS_SEG_MAX_SEGMENTS-1)
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      保存段元数据 (Magic + 段计数) 到 Flash 元数据页
//  @param      void
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      从 Flash 元数据页加载段计数, 含 Magic 校验
//  @param      void
//  @return     uint8   1=Magic 匹配数据有效, 0=Magic 不匹配数据无效
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      计算当前导航目标的距离和方位, 设置控制变量
//  @param      void
//  @return     void    输出写入全局变量 dis_ins, yaw_ins, Yao.Target_Speed, turn_mode
//-------------------------------------------------------------------------------------------------------------------
...
//  @brief      推进到段内下一个航点
//  @param      void
//  @return     uint8   1=当前段已完成(航点遍历完毕), 0=段内还有航点待执行
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      段间转移: 当前段航点遍历完毕后根据 arrival_behavior 决定下一步
//  @param      void
//  @return     uint8   1=全部导航完成(应退出), 0=继续执行(段/航点已更新)
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      分段编辑模式 (ins_mode=2): 检测按键执行航点录制/段管理操作
//  @param      void
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      分段导航模式 (ins_mode=3): 每帧执行目标计算、到达判定和段间转移
//  @param      void
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      完成当前段编辑并切换到下一段, 自动更新段计数和默认参数
//  @param      void
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      设置指定段的运行时参数 (速度/到达阈值/行为/事件ID)
//  @param      idx             段索引
//  @param      speed           目标速度
//  @param      threshold       到达判定距离阈值
//  @param      behavior        到达行为 (STOP/NEXT/RETURN/CUSTOM)
//  @param      custom_event_id 自定义事件ID, 仅 behavior==CUSTOM 时有效
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      保存所有段数据到 Flash (段数据页 + 元数据页)
//  @param      void
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      从 Flash 加载所有段到 RAM, 含 Magic 校验
//  @param      void
//  @return     uint8   1=加载成功, 0=Flash 数据无效
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      擦除所有段 Flash 数据并清空 RAM, 不可逆操作
//  @param      void
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      开始分段导航: 重置导航状态并切换 ins_mode=3
//  @param      void
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      停止导航: 归零速度/姿态/电机输出, 回退到平衡模式
//  @param      void
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
