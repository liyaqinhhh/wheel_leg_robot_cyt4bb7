...
uint8 steer_control_mode = 0; // 0:角度控制，灯亮开
                              // 1:PWM控制，递进方案，无速度环
uint8 turn_mode = 2;          // 0:关闭
                              // 1:递进PD转
                              // 2:串联转
                              // 3:yaw角闭环走直线
uint8 fuzzy_mode = 0;         // 0:关闭模糊，使用KP
                              // 1:开启模糊，使用KP作为最小，KI作为最大去模糊
uint8 menu_open = 1;          // 0:关闭菜单和flash以保证测试不会误写flash
                              // 1:打开菜单
                              // 2:只打开读取，不打开菜单
...
volatile float steer_vision_target_yaw_deg = 0.0f;       //视觉目标偏航角
volatile float steer_vision_cone_avoid_delta_deg = 0.0f; //视觉锥桶避障偏移角


volatile float steer_gps_target_bearing_deg = 0.0f; //GPS目标方位角
volatile float steer_gps_distance_to_wp_m = 0.0f;   //GPS距离
