...
{
    switch (leg)
    { // TCPWM_CH12_P05_3 TCPWM_CH21_P08_2
    case LF:
        return TCPWM_CH27_P19_2; // 前左腿 -> P01_1引脚 CH2 TCPWM_CH37_P21_5
    case LB:
        return TCPWM_CH36_P21_6; // 后左腿 -> P19_3引脚 CH1 TCPWM_CH28_P19_3
    case RF:
        return TCPWM_CH37_P21_5; // 前右腿 -> P01_0引脚 CH3
    case RB:
        return TCPWM_CH28_P19_3; // 后右腿 -> P00_3引脚 CH4
    default:
        return TCPWM_CH27_P19_2; // 默认返回前左腿通道
...
typedef struct
{
    ...
    float ref_angle;      // 中位PWM对应的物理角度（前腿202°,后腿-22°）
    float min_angle;      // 有效角度最小值
    float max_angle;      // 有效角度最大值
} ServoConfig;

// 各腿舵机参数（需根据实测校准）
// 新中位：LF=3690→202°, LB=5210→-22°, RF=5270→202°, RB=3900→-22°
static ServoConfig servo_cfg[] = {
    /* LF */ {3690, -30.0f, 202.0f, 90.0f, 270.0f}, // 前左腿：PWM增大角度减小
    /* LB */ {5210, -30.0f, -22.0f, -90.0f, 90.0f}, // 后左腿
    /* RF */ {5270, 30.0f, 202.0f, 90.0f, 270.0f},  // 前右腿：PWM增大角度增大
    /* RB */ {3900, 30.0f, -22.0f, -90.0f, 90.0f}   // 后右腿
};
...
void servo_set_angle(leg_enum leg, float angle)
{
    ...
    if (leg == LF || leg == RF)
    {
        if (mapped_angle < 90.0f)
            mapped_angle += 360.0f; // 处理-270度等效90度
    }
    else
    { // 后腿处理
        if (mapped_angle > 180.0f)
            mapped_angle -= 360.0f; // 转换为-180~180
    }
    ...
    // 4. 计算目标PWM（以 ref_angle 为水平参考点）
    float angle_offset = mapped_angle - cfg->ref_angle;

    uint16 pwm = (uint16)(cfg->mid_pwm + angle_offset * cfg->pwm_per_degree);

    // 5. PWM安全限幅
    pwm = (pwm < 1500) ? 1500 : (pwm > 7200) ? 7200
                                             : pwm;
...
void jump_control(void)
{
    ...
    if (time_j <= T1)
    {
        ...
        servo_set_angle(RB, 89);
        servo_set_angle(LF, 91);
        servo_set_angle(LB, 89);
    }
    // ===== 阶段2：收腿 =====
    else if (T1 <= time_j && time_j < (T1 + T2))
    {
        // 快速收缩腿部，前腿向后摆，后腿向前摆
        servo_set_angle(RF, 210);
        servo_set_angle(RB, -30);
        servo_set_angle(LF, 210);
        servo_set_angle(LB, -30);
    }
    // ===== 阶段3：放腿（起跳） =====
...
void servo_balance(void)
{
    ...
    er_X = (int16)PID_Increase(&PID_all.Pid_Inc_X, erect_Inc_X,
                               (float)(-motor_value.receive_left_speed_data + motor_value.receive_right_speed_data), Target_Speed);
    ...
    // 输出到舵机：对角腿反向运动产生扭腰效果
    servo_set_pwm(RF, er_X);
    servo_set_pwm(RB, -er_X);
    servo_set_pwm(LF, er_X);
    servo_set_pwm(LB, -er_X);
    // printf("平衡补偿PWM: %d\n", er_X);
}
...
float Single_Height = 7; // 单腿站立目标高度（cm）
