...
{
    int16 right_duty = -right_duty_1;
    int16 left_duty  = -left_duty_1;
    ...
    for(int i = 0; i < 6; i ++)
    {
     ...
     uart_write_buffer(SMALL_DRIVER_UART, motor_value.send_data_buffer, 7);   // 原串口发送，注释掉用于调试
    // ips200_show_string( 20, 0*8, "page: 333333333" );                        // 发送设置占空比的 字节包 数据

    // 屏幕显示发送数据（调试用）
    /*ips200_show_uint(0, 0*8, motor_value.send_data_buffer[0], 3);   // 帧头
    ips200_show_uint(0, 1*8, motor_value.send_data_buffer[1], 3);   // 功能字
    ips200_show_uint(0, 2*8, motor_value.send_data_buffer[2], 3);   // 左轮高八位
    ips200_show_uint(0, 3*8, motor_value.send_data_buffer[3], 3);   // 左轮低八位
    ips200_show_uint(0, 4*8, motor_value.send_data_buffer[4], 3);   // 右轮高八位
    ips200_show_uint(0, 5*8, motor_value.send_data_buffer[5], 3);   // 右轮低八位
    ips200_show_uint(0, 6*8, motor_value.send_data_buffer[6], 3);   // 校验和*/
}
...
//    uart_tx_interrupt(SMALL_DRIVER_UART, 1);
