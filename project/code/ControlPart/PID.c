...
/******************************************
// ???
        offset_angle.pitch =   2 ;    调小车往前倾，调大往后倾;
...
float erect_Gyro_Pitch[4] = {0.95, 0, 0, 0};  //  17.522  ,  0  ,  11  ,  0       // 陀螺仪比例: 1,0,0,0
float erect_Angle_Pitch[4] = {200, 0, 0}; //  17.49  ,  0  ,  9  ,  0        // 角度比例: 400,0,0,0
...
float erect_Angle_Yaw_3[4] = {0, 0, 0, 0}; //视觉
float erect_Angle_Yaw_4[4] = {0, 0, 0, 0}; //导航
// 运动学参数           Kp_x, Kp_y, Kp_roll
...
//----------------------------------------------------------------
//  @brief      闭环五连杆模型运动学逆解
//  @param      L1   面朝左侧时，左臂上连杆长度
//  @param      L2   面朝左侧时，左臂下连杆长度
//  @param      L3   面朝左侧时，右臂上连杆长度
//  @param      L4   面朝左侧时，右臂下连杆长度
//  @param      L5   面朝左侧时，上方横连杆长度
//  @return     void

//  @note       模型为车面朝左侧，故靠前方为左腿，后方为右腿，注意左右alpha和beta对应的舵机角度
//----------------------------------------------------------------
...
float X = 0, X_l = 0, Y = 5; // 确定Y值需先确定舵机中值
float temp_kx_x[4] = {0};
...
void Adapt_Terrain(void)
{
    ...
    if (Single_state == 1 && flag_Single_HighState == 0) // 站立
    {
        //        TCount_falg_4ms= 1;
...
float Cascade_angle_Pitch(PID_INFO *pid_info, float *PID_Parm, float NowPoint, float SetPoint)
{
    ...
    (void)temp_param; // 备用参数（对应代码已注释），消除未使用警告
    float V_out;
    float a = 0.9;
    ...
    // 2.低通滤波
    pid_info->iError = (1 - a) * pid_info->iError + a * pid_info->LastError; // 使得波形更加平滑，滤除高频干扰，防止突变
    pid_info->SumError += pid_info->iError;
