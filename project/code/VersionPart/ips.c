 ...
 * GPS导航专用屏幕显示函数
 *
 * 功能: 在IPS200屏幕上显示GPS导航测试所需的全部调试信息
 * 刷新: 10Hz (与gps_nav_proc同频), 局部刷新避免闪烁
 * 布局: 状态栏 → GPS原始数据 → 导航计算 → IMU姿态 → 航点信息
 *
 * 使用: 在main_cm7_0.c主循环中调用, 替代串口调试
 ********************************************************************************************************************/

/* GPS显示刷新控制 - 使用简单计数器降频, 避免依赖未链接的定时函数 */
static uint8 gps_show_decimate = 0;
#define GPS_SHOW_DECIMATE_CNT  3     // 主循环约30Hz调用, 每3次刷新1次≈10Hz
...
/**
 * @brief  GPS导航专用屏幕显示 (约10Hz局部刷新)
 * @note   主循环中调用, 内部3分频降频到约10Hz
 * @return 无
 */
